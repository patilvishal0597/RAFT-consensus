...
app.get("/getData", (req, res) => {
  res.json(onRetrieveRequest(req))
})
  ...
  socketServer.on('message',function(msg, rinfo) {
    ...
    if (nodeIsAlive) {
      if (msg.request === 'VOTE_REQUEST') {
        ...
        destination = msg.sender_name
        const newMessage = onRetrieveRequest(msg)
        sender(socketServer, destination, newMessage)
      }
...
function onRetrieveRequest(msg) {
  if(node.state === STATES.LEADER){
    destination = msg.sender_name
